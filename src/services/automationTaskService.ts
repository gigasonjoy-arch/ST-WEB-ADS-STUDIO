  ...
  public static generateTasks(params: {
    ...
    if (newLeads.length > 0) {
      ...
      tasks.push({
        ...
        category: 'LEAD',
        targetTab: 'LEADS',
        targetSubTab: 'NEW_LEADS',
        targetElementId: `lead-card-${urgentLead.id}`,
        targetParams: { filterStatus: 'NEW', leadId: urgentLead.id },
    ...
    if (unresolvedGaps.length > 0) {
      ...
      tasks.push({
        ...
        targetSubTab: 'UNRESOLVED',
        targetElementId: `gap-item-${topGap.id}`,
        targetParams: { filter: 'UNRESOLVED', gapId: topGap.id, query: gapQuery },
    ...
    if (missingMetrics.length > 0) {
      ...
      tasks.push({
        ...
        targetSubTab: 'BENCHMARKS_TABLE',
        targetElementId: 'benchmarks-table-view',
        targetParams: { highlightCategory: sampleCategory },
    ...
    if (priceRanges.length < 3) {
      tasks.push({
        ...
        category: 'BENCHMARK',
        targetTab: 'CALCULATOR_BENCHMARKS',
        targetSubTab: 'PRICE_RANGES',
        targetElementId: 'price-ranges-section',
        targetParams: { subTab: 'PRICE_RANGES' },
    ...
    if (draftCaseStudies.length > 0) {
      tasks.push({
        ...
        targetSubTab: 'DRAFT_LIST',
        targetElementId: `case-study-${draftCaseStudies[0].id}`,
        targetParams: { filter: 'DRAFT', caseStudyId: draftCaseStudies[0].id },
    ...
    if (isFirebaseWorking === false) {
      tasks.push({
        ...
        category: 'FIREBASE',
        targetTab: 'SETTINGS',
        targetSubTab: 'FIREBASE_TEST',
        targetElementId: 'firebase-diagnostics-section',
        targetParams: { subTab: 'FIREBASE_TEST' },
    ...
    if (!hasGoogleSheetLinked) {
      tasks.push({
        ...
        targetSubTab: 'GOOGLE_SHEETS',
        targetElementId: 'google-sheets-sync-card',
        targetParams: { subTab: 'GOOGLE_SHEETS' },
    ...
    if (!siteSettings?.exchangeRateUsdToBdt || siteSettings.exchangeRateUsdToBdt < 100) {
      tasks.push({
        ...
        targetElementId: 'setting-exchange-rate-input',
        targetParams: { subTab: 'GENERAL', elementId: 'setting-exchange-rate-input' },
    ...
    // 9. HEADER & LOGO BRANDING CONFIGURATION CHECK
    if (!siteSettings?.header || !siteSettings?.header?.logoDisplayMode) {
      tasks.push({
        id: 'task-header-config',
        title: 'Configure Header Logo & Brand Visibility Rules',
        titleBn: 'হেডার লোগো এবং ব্র্যান্ড দৃশ্যমানতা কনফিগার করুন',
        description: 'Set desktop & mobile header visibility (Logo only, Name only, or Both).',
        descriptionBn: 'হেডারে লোগো এবং কোম্পানির নাম কীভাবে দেখাবে (ডেস্কটপ ও মোবাইলে) তা অ্যাডমিন প্যানেল থেকে পছন্দমতো সাজিয়ে নিন।',
        reason: 'A customized header layout delivers a tailored branding experience across devices.',
        reasonBn: 'মোবাইল ও পিসিতে সঠিক লোগো প্রেজেন্টেশন ক্লায়েন্টদের বিশ্বাসযোগ্যতা বহুগুণ বাড়ায়।',
        priority: 'MEDIUM',
        category: 'SETTINGS',
        targetTab: 'SETTINGS',
        targetSubTab: 'HEADER_MANAGEMENT',
        targetElementId: 'header-management-panel',
        targetParams: { subTab: 'HEADER_MANAGEMENT' },
        actionLabel: 'Customize Header & Logo',
        actionLabelBn: 'হেডার ও লোগো কন্ট্রোলে যান',
        iconName: 'Layout',
        createdAt: now
      });
    }
